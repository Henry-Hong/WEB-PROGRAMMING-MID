...
export class Particle {
  constructor(x, y, color, stageWidth, stageHeight) {
    ...
    this.toOriginDist = Math.sqrt((this.originX - this.startX) ** 2 + (this.originY - this.startY) ** 2);
    this.toRanDist = Math.sqrt((this.originX - this.startX) ** 2 + (this.originY - this.startY) ** 2);
  ...
  draw(ctx) {
    ...
    if ((Math.abs(this.vx) > 0 && Math.abs(this.vx) < 0.001) || (Math.abs(this.vy) > 0 && Math.abs(this.vy) < 0.001)) {
      this.startX = this.cntX;
      this.startY = this.cntY;
      this.toRanDist = Math.sqrt((this.ranX - this.startX) ** 2 + (this.ranY - this.startY) ** 2);
      this.vx = 0;
      this.vy = 0;
    }
    ...
    const g = ctx.createRadialGradient(this.cntX, this.cntY, this.radius * 0.2, this.cntX, this.cntY, this.radius);
    ...
    if (this.flying) {
      ...
      g.addColorStop(1, this.lightColor + this.decToHex(Math.floor(this.light / 5)));
    }
    ctx.fillStyle = g;
