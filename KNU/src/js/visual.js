...
export class Visual {
  constructor(pixeles, stageWidth, stageHeight, pixelRatio) {
    ...
    this.frame = 60;
    this.acceleration = 1 + 0.006 * this.reversePixelRatio;
  ...
  setParticles() {
    this.pixeles.forEach((pixel) => {
      const particle = new Particle(pixel.x, pixel.y, pixel.color, this.stageWidth, this.stageHeight, this.isMouseDown);
  ...
  onMouseDown() {
    ...
    this.particles.forEach((particle) => {
      ...
      particle.toOriginDist = Math.sqrt((particle.originX - particle.startX) ** 2 + (particle.originY - particle.startY) ** 2);
  ...
  onMouseUp() {
    ...
    this.particles.forEach((particle) => {
      ...
      particle.startX = particle.cntX;
      particle.startY = particle.cntY;
      particle.toRanDist = Math.sqrt((particle.ranX - particle.startX) ** 2 + (particle.ranY - particle.startY) ** 2);
